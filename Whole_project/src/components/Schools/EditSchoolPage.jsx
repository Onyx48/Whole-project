...
import { parse, format, isValid } from "date-fns"; // Make sure format is imported for reset values
...
function EditSchoolPage({ schoolToEdit, onUpdateSchool, onClose }) {
  ...
  useEffect(() => {
    if (schoolToEdit) {
      // Backend stores dates as ISO strings (or Date objects), so convert to Date objects for DatePicker
  ...
  const handleDiscardOrClose = () => {
    // Resetting to the original values of schoolToEdit is usually desired for "Discard Changes"
